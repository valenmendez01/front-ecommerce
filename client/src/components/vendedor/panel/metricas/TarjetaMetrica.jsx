...
const TarjetaMetrica = ({ titulo, valor, descripcion, Icono, destacar }) => {
  return (
    <Card className="border border-dorado-primary/30 shadow-lg" radius="sm">
      <CardBody className="flex min-h-32 flex-row items-center justify-between gap-4 px-6 py-5">
        <div>
          ...
          <p className={`mt-2 text-3xl font-black ${destacar ? 'text-dorado-primary' : 'text-green-primary'}`}>
            {valor}
          </p>
          <p className="mt-1 text-sm text-slate-500">{descripcion}</p>
        </div>
        {Icono && (
          <div className="flex h-12 w-12 items-center justify-center rounded-md bg-dorado-primary/20 text-green-primary">
